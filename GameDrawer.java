...
public class GameDrawer extends JPanel implements KeyListener, ActionListener{
    ...
    public void paint(Graphics g){
        ...
        if(fruitEaten > bestScore)
            bestScore = fruitEaten;
 
        g.setColor(new Color(243,234,194));
        g.fillRect(0, 0, 750, 500);
 
        g.setColor(new Color(236,82,75));
        for(int i=6; i<=482; i+=17)
            for(int j=6; j<=482; j+=17)
                g.fillRect(i, j, 13, 13);
 
        g.setColor(new Color(154,211,188));
        g.fillRect(20, 20, 460, 460);
 
        g.setColor(new Color(236,82,75));
        g.setFont(new Font("Arial", Font.BOLD, 26));
        g.drawString("لعبة الثعبان", 565, 70);
 
      
        g.setColor(new Color(234,167,82));
 
        FontMetrics fm = g.getFontMetrics();
 
        g.setFont(new Font("Arial", Font.PLAIN, 18));
 
        g.drawString("أفضل عدد للتفاحات المأكوله", 540, 110);
        g.drawRect(550, 120, 140, 30);
        g.drawString(bestScore+"", 550+(142-fm.stringWidth(bestScore+""))/2, 142);

 
        g.drawString("التفاحات المأكولة", 560, 170);
        g.drawRect(550, 180, 140, 30);
        g.drawString(fruitEaten+"", 550+(142-fm.stringWidth(fruitEaten+""))/2, 202);
 
        g.setFont(new Font("Arial", Font.BOLD, 16));
        g.drawString("التحكم", 600, 250);
 
        g.setFont(new Font("Arial", Font.PLAIN, 14));
        g.drawString("للوقوف والأستمرار : مساحه", 570, 270);
        g.drawString(" للأعلى : سهم الأعلى", 590, 300);
        g.drawString(" للأسفل : سهم الأسفل", 590, 330);
        g.drawString("لليسار :  سهم اليسار", 590, 360);
        g.drawString("لليمين : سهم اليمين", 590, 390);
        ...
        for(int i=1; i<lengthOfSnake; i++) {
            if(boardX[i] == boardX[0] && boardY[i] == boardY[0]) {
                ...
                g.drawString("خلصنا مع السلامة ", 110, 220);
 
                g.setFont(new Font("Arial", Font.BOLD, 20));
                g.drawString("حط مساحه لو تبي تعيد اللعبه ", 130, 260);
